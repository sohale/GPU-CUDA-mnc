...
//CUDA code to calculate the CDF of Gaussian using Monte Carlo.
...
__device__ size_t getGlobalIndex() {
// block number
...
/** The actual MVN CDF algorithm for a single location */
__device__
double transf(
const double * w,
const double * b,
...
//where on global memory the current thread reads and writes.
// Then it calls transf to do the actual work.

__global__
void processMCSample(
double * out,
const double *wa,
const double *b,
const double *C,
